 ...
 */
const filtersToSolrQuery = (fields=['content_txt_fr']) => {
  return async context => {
    ...
    let queries = (context.params.sanitized.filters || []).filter(d => d.type == 'string');
    ...
    const sq = queries.reduce((sq, query) => {
      ...
      let _q = query.q.trim();
      let _isExact = /^"[^"]+"$/.test(_q);
      let _hasSpaces = _q.split(' ').length > 1;
