...
/**
 * Interface for Solr configuration mappings
 *
 * This interface defines the configuration structure for mapping
 * client-side facet types to Solr query parameters.
 */
interface ISolrMappings<F extends FilterType = FilterType> {
  /**
   * Mapping of facet types to Solr facet query parameters
   * Each key corresponds to a specific facet type that is supported by the index
   */
  facets: Record<F, SolrFacetQueryParams>

  /**
   * Field mappings for ordering results
   * Key is the client-facing name, value is the Solr field name
   */
  orderBy?: Record<string, string>

  /**
   * Field mappings for grouping results
   * Key is the client-facing name, value is the Solr field name
   */
  groupBy?: Record<string, string>

  /**
   * Default query parameters for this index
   */
  defaultParams?: Record<string, string | number | boolean>
}

/**
 * Facets available in the search index.
 */
export type ContentItemFacet = Extract<
  FilterType,
  | 'daterange'
  | 'year'
  | 'contentLength'
...
} satisfies ISolrMappings<ContentItemFacet>
...
const trClustersSolrMappings = {
  facets: {
    ...
    } as const satisfies SolrRangeFacetQueryParams,
  },
} satisfies ISolrMappings<TextReuseClusterFacet>
 ...
 */
export const SolrMappings = Object.freeze({
  search: searchSolrMappings,
...
export const Indexes = ['search', 'tr_clusters', 'tr_passages', 'images'] satisfies IndexType[]
