...
class Service implements SearchService {
  ...
  /**
   * @deprecated it's a proxy for a method in 'collectable-items' service. It's still here to support
   * the UI.
   * Save current search and return the corrseponding searchQuery
   */
  async create(data: any, params: any) {
    await this.app.service('collectable-items').create(
      {
        filters: data.sanitized.filters,
        namespace: data.sanitized.index,
      },
      {
        route: {
          collection_id: data.sanitized.collection_uid,
        },
        user: params.user,
      }
    )
