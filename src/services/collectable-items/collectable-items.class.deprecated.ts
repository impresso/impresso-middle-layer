...
import { Service as DeprecatedCollectionsService } from '../collections/collections.class.deprecated'
...
export class Service {
  ...
  private get _collectionsService() {
    // retrofit to old collections service
    return this.app.service('collections') as any as DeprecatedCollectionsService
  }
  ...
  async find(params: Params<FindQuery> & Sanitized<FindQuery> & WithUser) {
    ...
    const collectionIds: string[] = Array.from(new Set(results.data.flatMap((d: any) => d.collectionIds)))
    debug('[find] collectionIds:', collectionIds)

    const collections = await this._collectionsService
      .findInternal({
        user: params.user,
        query: {
          uids: collectionIds,
  ...
  async create(data: any, params: Params & WithUser) {
    ...
    const collection = await this._collectionsService.get(data.sanitized.collection_uid, {
      user: params.user,
    })
    if (!collection) {
      throw new NotFound()
  ...
  async remove(id: string, params: Params & Sanitized<any> & WithUser) {
    // get collection, only if it does belongs to the user
    const collection = await this._collectionsService.get(params.sanitized.collection_uid, {
      user: params.user,
    })
    if (!collection) {
      throw new NotFound()
