...
export interface IRateLimiter {
   ...
   */
  allow(userId: string, resource: string): Promise<boolean>;
  /**
...
class RateLimiter implements IRateLimiter {
  ...
  async allow(userId: string, resource: string): Promise<boolean> {
    if (this.rateLimiterScriptSha == null) throw new Error('Rate limiter not initialized');

    const result = await this.redisClient.evalSha(this.rateLimiterScriptSha, {
