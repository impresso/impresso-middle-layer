...
class Service {
  ...
  async find(params) {
    const where = {
      ...
      [Op.and]: [
        {
          [Op.or]: [{ creatorId: params.user.id }, { status: Collection.STATUS_PUBLIC }],
        },
      ],
    }
    ...
    if (params.query.q) {
      where[Op.and].push({
        [Op.or]: [{ name: params.query.q }, { description: params.query.q }],
      })
    }
  ...
  async get(id, params) {
    ...
    const transform = params.query.nameOnly ? c => pick(c, ['uid', 'name', 'description']) : identity
