...
class Service {
  ...
  async assignIIIF ({ method, result }) {
    ...
    if (!pages.length || pages.length !== uids.length) {
      debug(
        'assignIIIF: cannot find some pages, requested:',
        uids,
        'found:',
        pages
      )
  ...
  async find (params) {
    debug(
      `[find] with params.isSafe:${params.isSafe} and params.query:`,
      params.query
    )
    let signature
    if (params.query.similarTo) {
      ...
      if (!signature) {
        debug(
          '[find] signature NOT retrieved for params.query.similarTo:',
          params.query.similarTo
        )
        throw new NotFound('signature not found')
      } else {
        debug(
          '[find] signature retrieved for params.query.similarTo:',
          params.query.similarTo
        )
      }
  ...
  async get (id, params) {
    debug(
      `get '${this.name}': with params.isSafe:${params.isSafe} and params.query:`,
      params.query
    )
    return measureTime(
      () =>
